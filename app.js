...
const divItems = resultadosCalculo.map(resultado => {
    const divItem = document.createElement('div');
...
// Utilizando map para agregar al DOM
const divsFragment = document.createDocumentFragment();
divItems.map(divItem => divsFragment.appendChild(divItem));
resultadosEficienciaDiv.appendChild(divsFragment);


function BuscarEficienciaCombustible() {
    const camionIDBuscado = parseInt(document.getElementById('idCamion').value, 10);
...
/* function buscarPaquete() {
    ...
    const idpaqueteBuscado = parseInt(idpaqueteInput.value, 10);

    try {
        isNaN(idpaqueteBuscado)
            ? (() => { throw new Error("Por favor, ingresa un número válido para el ID del paquete."); })()
            : seguimientoPaquete(camiones, paquetes, idpaqueteBuscado);
    } catch (error) {
